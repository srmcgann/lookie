<?
	if(isset($_POST['scroll'])){
		...
		$imagesPerScroll=12;
		$sql="SELECT * FROM images ORDER BY $sort DESC";
		$res=$link->query($sql);
		$count=0;
				...
				if($count>$_POST['scroll']*$imagesPerScroll && $count<=$_POST['scroll']*$imagesPerScroll+$imagesPerScroll){
					...
					switch($row['type']){
						...
						case "video/webm":
							...
							break;
						case "video/ogg":
							$fileName="../uploads/$base.ogg";
							$type="vid";
							break;
					}
					$thumb="$fileName.jpg";
					list($width, $height) = getimagesize($thumb);
					echo "<div class='crop' id='$shortName' style='background-image:url($thumb);width:".$width."px' onclick=\"view('$thumb','$type','$shortName')\"><img class='hover' src='".($type=="pic"?"pic.png":"vid.png")."'></div> ";
				}
			}
		}
	}
